...
def Guess():
    ...
    word_copy = list(word)  # Create a copy of the target word
            ...
            if user_guess == word:
                ...
                for i, guessLetter in enumerate(user_guess):
                    ...
                    if guessLetter == word[i]:
                        guessLabel.config(bg=green, fg=grey)
                        word_copy[i] = None
                    elif guessLetter in word_copy:
                        guessLabel.config(bg=yellow, fg=grey)
                        word_copy[word_copy.index(guessLetter)] = None
                    else:
